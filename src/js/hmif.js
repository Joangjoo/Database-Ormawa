...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("https://api-database-ormawa-production.up.railway.app/hmif")
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        data.slice(0, 6).forEach((item, index) => {
          ...
          InnerCard2.className =
            "absolute peer-hover:-top-20 peer-hover:-left-16 peer-hover:w-[140%] peer-hover:h-[140%] -top-32 -left-16 w-32 h-44 rounded-full bg-bg-lapisan transition-all duration-500 ";
  
          const InnerCard3 = document.createElement("div");
          InnerCard3.className =
            "absolute flex flex-col text-xl text-center items-end justify-end peer-hover:right-0 peer-hover:rounded-b-none peer-hover:bottom-0 peer-hover:items-center peer-hover:justify-center peer-hover:w-full peer-hover:h-full -bottom-32 -right-16 w-32 h-44 rounded-full bg-bg-lapisan transition-all duration-500";
